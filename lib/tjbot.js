...
function TJBot(hardware, configuration, credentials) {
    ...
    hardware.forEach(function(device) {
        switch (device) {
            ...
            case 'leftArm':
                this._setupLeftArm(this.configuration.leftArm.servoPin);
                break;

            case 'rightArm':
                this._setupRightArm(this.configuration.rightArm.servoPin);
                break;
 ...
 */
TJBot.prototype.version = 'v1.2.2 Two Arms Two LEDs One Sonar';

/**
 * List of TJBot hardware and services.
 */
TJBot.prototype.capabilities = ['analyze_tone', 'converse', 'listen', 'see', 'shine', 'shineLeftRight', 'speak', 'translate', 'wave', 'leftArm', 'rightArm'];
TJBot.prototype.hardware = ['camera', 'led', 'twoLeds', 'microphone', 'servo', 'speaker', 'leftArm', 'rightArm'];
...
TJBot.prototype.defaultConfiguration = {
    ...
    leftArm: { //adding left arm
        servoPin: 7 // corresponds to BCM 7 / physical PIN 26
    },
    rightArm: { // adding right arm
        servoPin: 25 // corresponds to BCM 25 / physical PIN 22
    },
    speak: {
...
TJBot.prototype._setupLED = function() {
    ...
    // init with 1 LED
    this._led = ws281x;
    //this._led.init(1); add second led support tx to Lisa Waugh
    this._led.init(2);
...
/** 
 * Configure the servo module that manages left arm for the given pin number.
 *
 * @param {Int} pin The pin number to which the servo is connected.
 */
TJBot.prototype._setupLeftArm = function(pin) {
    var Gpio = require('pigpio').Gpio;

    winston.verbose("TJBot initializing left arm servo motor on PIN " + pin);

    this._motorLeftArm = new Gpio(pin, {
        mode: Gpio.OUTPUT
    });
}

/** 
 * Configure the servo module that manages right arm for the given pin number.
 *
 * @param {Int} pin The pin number to which the servo is connected.
 */
TJBot.prototype._setupRightArm = function(pin) {
    var Gpio = require('pigpio').Gpio;

    winston.verbose("TJBot initializing left arm servo motor on PIN " + pin);

    this._motorRightArm = new Gpio(pin, {
        mode: Gpio.OUTPUT
    });
}
...
TJBot.prototype._assertCapability = function(capability) {
    switch (capability) {
        ...
        case 'leftArm': //adding leftArm
            if (!this._motorLeftArm) {
                throw new Error(
                    'TJBot is not configured with a left arm. ' +
                    'Please check you included the "leftArm" hardware in the TJBot constructor.');
            }
            break;
			
        case 'rightArm': //adding rightArm
            if (!this._motorRightArm) {
                throw new Error(
                    'TJBot is not configured with a right arm. ' +
                    'Please check you included the "rightArm" hardware in the TJBot constructor.');
            }
            break;
	}
}
...
/**
 * Take a picture and identify the persons present.
 *
 * Returns a list of person seen and their confidences.
 * See VisualRecognitionV3.prototype.classify for more detail on the
 * return object.
 */
TJBot.prototype.recognize = function() {
    this._assertCapability('see');
    ...
    return new Promise(function(resolve, reject) {
        winston.verbose("TJBot taking a photo");
        self.takePhoto().then(function(filePath) {
            resolve(self.recognizeFacesInPhoto(filePath))
        });
    });
}
...
/*
Describe persons in photo by sending it to the Watson Visual Recognition Service.
 */
TJBot.prototype.recognizeFacesInPhoto = function(filePath) {
    this._assertCapability('see');
    ...
    return new Promise(function(resolve, reject) {
        winston.debug("sending image to Watson Visual Recognition to recognize faces");
        var params = {
            images_file: fs.createReadStream(filePath),
            threshold: self.configuration.see.confidenceThreshold.object
        };

        self._visualRecognition.detectFaces(params, function(err, response) {
            if (err) {
                reject(err);
            } else {
                var result = response.images[0].faces;
                resolve(result);
            }
        });
    });
...
TJBot.prototype.shine = function(color) {
    ...
    // set the LED color
    //var colors = new Uint32Array(1); 	// add 2 leds support tx Lisa Waugh 
    //colors[0] = parseInt(grb); 		// add 2 leds support tx Lisa Waugh 
    var colors = new Uint32Array(2); 	// add 2 leds support tx Lisa Waugh 
    colors[0] = parseInt(grb); 			// add 2 leds support tx Lisa Waugh 
    colors[1] = parseInt(grb); 			// add 2 leds support tx Lisa Waugh
 ...
 * @param {String} color The color to use. Must be interpretable by TJBot.prototype._normalizeColor.
 */
TJBot.prototype.shineLeftRight = function(colorLeft, colorRight) {
    this._assertCapability('shineLeftRight');

    // convert to rgb
    var rgbLeft = this._normalizeColor(colorLeft);
    var rgbRight = this._normalizeColor(colorRight);
	
    // convert hex to the 0xGGRRBB format for the LED
    var grbLeft = "0x" + rgbLeft[3] + rgbLeft[4] + rgbLeft[1] + rgbLeft[2] + rgbLeft[5] + rgbLeft[6];
    var grbRight = "0x" + rgbRight[3] + rgbRight[4] + rgbRight[1] + rgbRight[2] + rgbRight[5] + rgbRight[6];
	
    // shine!
    winston.verbose("TJBot shining my Left LED to RGB color " + rgbLeft);
    winston.verbose("TJBot shining my Right LED to RGB color " + rgbRight);
	
    // set the LED color
    //var colors = new Uint32Array(1); 	// add 2 leds support tx Lisa Waugh 
    //colors[0] = parseInt(grb); 		// add 2 leds support tx Lisa Waugh 
    var colors = new Uint32Array(2); 	// add 2 leds support tx Lisa Waugh 
    colors[0] = parseInt(grbLeft); 			// add 2 leds support tx Lisa Waugh 
    colors[1] = parseInt(grbRight); 			// add 2 leds support tx Lisa Waugh
    this._led.render(colors);
}
...
// adding for additional TJBot left arm
/** ------------------------------------------------------------------------ */
/** WAVE LEFT ARM                                                                   */
/** ------------------------------------------------------------------------ */
...
TJBot.prototype._SERVO_ARM_BACK = 500;
TJBot.prototype._SERVO_ARM_UP = 1400;
TJBot.prototype._SERVO_ARM_DOWN = 2200;

/**
 * Move TJ's left arm all the way back.
 */
TJBot.prototype.leftArmBack = function() {
    // make sure we have a left arm
    this._assertCapability('leftArm');
    this._motorLeftArm.servoWrite(TJBot.prototype._SERVO_ARM_BACK);
}

/**
 * Raise TJ's  arm.
 */
TJBot.prototype.raiseLeftArm = function() {
    // make sure we have a left arm
    this._assertCapability('leftArm');
    this._motorLeftArm.servoWrite(TJBot.prototype._SERVO_ARM_UP);
}

/**
 * Lower TJ's left arm.
 */
TJBot.prototype.lowerLeftArm = function() {
    // make sure we have a left arm
    this._assertCapability('leftArm');
    this._motorLeftArm.servoWrite(TJBot.prototype._SERVO_ARM_DOWN);
}

/**
 * Wave TJ's left arm.
 */
TJBot.prototype.waveLeftArm = function() {
    this._assertCapability('leftArm');
...
// adding for additional TJBot right arm
/** ------------------------------------------------------------------------ */
/** WAVE RIGHT ARM                                                                   */
/** ------------------------------------------------------------------------ */

TJBot.prototype._SERVO_RIGHT_ARM_BACK = 2200; 	//review this value I'm not an expert in servo moving :)
TJBot.prototype._SERVO_RIGHT_ARM_UP = 1400;		//review this value I'm not an expert in servo moving :)
TJBot.prototype._SERVO_RIGHT_ARM_DOWN = 500;	//review this value I'm not an expert in servo moving :)

/**
 * Move TJ's left arm all the way back.
 */
TJBot.prototype.rightArmBack = function() {
    // make sure we have a right arm
    this._assertCapability('rightArm');
    this._motorRightArm.servoWrite(TJBot.prototype._SERVO_RIGHT_ARM_BACK);
}

/**
 * Raise TJ's right arm.
 */
TJBot.prototype.raiseRightArm = function() {
    // make sure we have a rightArm');
    this._motorRightArm.servoWrite(TJBot.prototype._SERVO_RIGHT_ARM_UP);
}

/**
 * Lower TJ's right arm.
 */
TJBot.prototype.lowerRightArm = function() {
    // make sure we have a right arm
    this._assertCapability('rightArm');
    this._motorRightArm.servoWrite(TJBot.prototype._SERVO_RIGHT_ARM_DOWN);
}

/**
 * Wave TJ's right arm.
 */
TJBot.prototype.waveRightArm = function() {
    this._assertCapability('RightArm');
 ...
 * Wave TJ's arms.
 */
TJBot.prototype.waveArms = function() {
    this._assertCapability('RightArm');
    this._assertCapability('LeftArm');
